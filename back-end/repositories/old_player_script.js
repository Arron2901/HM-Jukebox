...
let openid;
let status = "off";
const songNameLength = 25;
const artistNameLength = 38;
let popup = null;
...
function getToken(status){
  ...
  const clientId = '45e29ed600ae4df4903a380c952d2a12';
...
function getPlaylists(){
  ...
  fetch('https://api.spotify.com/v1/browse/categories/throwback/playlists', {
	   method: 'get',
     headers: {
       "authorization": "Bearer " + _token
     }
...
function trackSearch(){
  let query = document.getElementById('keyboard').value;
  console.log(query);
...
function populateTracks(){
  ...
  document.getElementById('search-results').innerHTML = "";
   for (let i = 0; i<tracks.length; i++){
     document.getElementById('search-results').innerHTML += '<div class="result" onclick="openPopup('+i+')"><img src="'+tracks[i].album.images[1].url+'" class="album-art" alt=""><div class="song-info"><p id="track-name">'+tracks[i].name+'</p><p id="artist-name">'+tracks[i].album.artists[0].name+'</p></div></div>'
   }
}
...
function updatePlaylistWindow(){
  if (playlist.length > 0) {
    ...
    for (let i = 0; i < playlist.length; i++){
      ...
      document.getElementById('playlist-window').innerHTML += '<div class="playlist-item"><img src="'+playlist[i].cover+'" alt=""><div class="playlist-item-info"><p id="pl_song">'+name+'</p><p id="pl_artist">'+artist+'</p></div></div>';
    }
  }
  else {
